...
static int wmt_spi_regs_config(struct spi_device *spi_dev)
{
	...
	*/
	...
	//sand add it
	//dev_info(&wmt_spi->pdev->dev, "%x %x %x", spi_get_reg32(wmt_spi, 0), spi_get_reg32(wmt_spi, 4), spi_get_reg32(wmt_spi, 8));
	return 0;
		...
		if (rx) {
			for (i = 0; i < SPI_FIFO_SIZE; i++)
			{
				...
				*rx++ = spi_read_fifo(wmt_spi, SPI_RXFIFO);
				//sand add it
	            //dev_info(&spi->dev, "master rx buff :%x", *(rx - 1));
	        }
		}
		/* check if rx finish */
...
wmt_spi_dma_write_read(struct spi_device *spi_dev, struct spi_transfer *t)
{
	...
	dev_info(&spi_dev->dev, "(i)%x %x %x", spi_get_reg32(wmt_spi, 0), spi_get_reg32(wmt_spi, 4), spi_get_reg32(wmt_spi, 8));
	/* reset spi fifo */
	if (spi_reset(wmt_spi))
		goto out;
